...
class UserExample : ExampleBase
{
    public override bool Content(Canvas canvas, uint w, uint h)
    {
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            if (!Verify(picture.Load(ExamplePaths.ExampleDir + "/image/test.png"))) return false;
        ...
        //Open file manually
        var fileData = File.ReadAllBytes(ExamplePaths.ExampleDir + "/image/test.png");

        var picture2 = Picture.Gen();
        if (!Verify(picture2.Load(fileData, (uint)fileData.Length, "png", "", true))) return false;
        picture2.Translate(380, 0);
        picture2.Scale(0.8f);
        canvas.Add(picture2);
