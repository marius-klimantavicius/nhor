...
public class GridFitContentPercentIndefiniteArgument
{
    [Fact]
    public void BorderBox()
    {
        var taffy = NewTestTree();
        var node0 = taffy.NewLeafWithContext(new Style(), TestNodeContext.AhemText("HH\u200bHH\u200bHH", WritingMode.Horizontal));
        var node = taffy.NewWithChildren(new Style
        {
            Display = Display.Grid,
            GridTemplateRows = ImmutableList.Create(GridTemplateComponent.FromLength(40f)),
            GridTemplateColumns = ImmutableList.Create(GridTemplateComponent.FitContentComponent(LengthPercentage.Percent(0.5f))),
        }, new NodeId[] { node0 });
