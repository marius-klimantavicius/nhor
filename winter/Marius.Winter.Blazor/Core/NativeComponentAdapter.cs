...
namespace Marius.Winter.Blazor.Core
{
    ...
    internal sealed class NativeComponentAdapter : IDisposable
    {
        ...
        internal void ApplyEdits(int componentId, ArrayBuilderSegment<RenderTreeEdit> edits, ArrayRange<RenderTreeFrame> referenceFrames, RenderBatch batch, HashSet<int> processedComponentIds)
        {
            ...
            // StepIn/StepOut navigate into child adapters. Track the current
            // target adapter with a stack so edits apply to the right level.
            //
            // In the adapter tree, component adapters that have _targetElement collapse
            // two Blazor tree levels into one: the component level and the element level.
            // When Blazor says "StepIn sibling=0" at the component level, it means "enter
            // the element at sibling 0" — but in the adapter tree that element IS the
            // current adapter. So we must NOT navigate to Children[0]. Instead we stay at
            // the same adapter and mark that we're now "inside the element". A subsequent
            // StepIn from that level does navigate to Children[siblingIndex] normally.
            var currentAdapter = this;
            var insideElement = false;
            var stateStack = new Stack<(NativeComponentAdapter adapter, bool insideElement)>();
            List<(int from, int to)>? permutationList = null;
            ...
            foreach (var edit in edits)
            {
                switch (edit.Type)
                {
                    case RenderTreeEditType.PrependFrame:
                        currentAdapter.ApplyPrependFrame(batch, componentId, edit.SiblingIndex, referenceFrames.Array, edit.ReferenceFrameIndex, processedComponentIds);
                        break;
                    case RenderTreeEditType.RemoveFrame:
                        currentAdapter.ApplyRemoveFrame(edit.SiblingIndex);
                        break;
                    case RenderTreeEditType.SetAttribute:
                        currentAdapter.ApplySetAttribute(ref referenceFrames.Array[edit.ReferenceFrameIndex]);
                        break;
                    case RenderTreeEditType.RemoveAttribute:
                        currentAdapter.ApplyRemoveAttribute(edit.RemovedAttributeName!);
                        break;
                    case RenderTreeEditType.UpdateText:
                        {
                            var frame = batch.ReferenceFrames.Array[edit.ReferenceFrameIndex];
                            if (currentAdapter._targetElement is IHandleChildContentText handleChildContentText)
                            {
                    ...
                    case RenderTreeEditType.StepIn:
                        stateStack.Push((currentAdapter, insideElement));
                        if (currentAdapter._targetElement != null && !insideElement)
                        {
                            // Entering the element that IS the current adapter — stay here.
                            insideElement = true;
                        }
                        else
                        {
                            currentAdapter = currentAdapter.Children[edit.SiblingIndex];
                            insideElement = false;
                        }
                        break;
                    case RenderTreeEditType.StepOut:
                        {
                            ...
                            currentAdapter = state.adapter;
                            insideElement = state.insideElement;
                        }
                        break;
                    ...
                    case RenderTreeEditType.PermutationListEntry:
                        // MoveToSiblingIndex shares the field offset with ReferenceFrameIndex
                        permutationList ??= new List<(int, int)>();
                        permutationList.Add((edit.SiblingIndex, edit.ReferenceFrameIndex));
                        break;
                    case RenderTreeEditType.PermutationListEnd:
                        currentAdapter.ApplyPermutations(permutationList!);
        ...
        private void ApplyPermutations(List<(int from, int to)> permutationList)
        {
            if (permutationList == null || permutationList.Count == 0)
                return;

            // Phase 1: Snapshot adapters at their current (from) positions.
            var snapshots = new NativeComponentAdapter[permutationList.Count];
            for (int i = 0; i < permutationList.Count; i++)
            {
                snapshots[i] = Children[permutationList[i].from];
            }

            // Phase 2: Place each adapter at its target (to) position.
            for (int i = 0; i < permutationList.Count; i++)
            {
                Children[permutationList[i].to] = snapshots[i];
            }

            // Phase 3: Reorder physical elements to match the new logical order.
            var physicalParent = _targetElement ?? _closestPhysicalParent;
            if (physicalParent == null)
                return;

            var physicalChildren = new List<IElementHandler>();
            CollectPhysicalChildren(this, physicalChildren);

            // Use ReorderChildren to swap children in-place without Scene.Remove/Add
            // (which would break ThorVG ref counts and parent pointers).
            if (physicalParent is IWinterContainerElementHandler container)
            {
                var elements = new List<Element>();
                foreach (var ph in physicalChildren)
                {
                    if (ph is IWinterElementHandler wh)
                        elements.Add(wh.ElementControl);
                }
                container.ReorderChildren(elements);
            }
        }

        private static void CollectPhysicalChildren(NativeComponentAdapter adapter, List<IElementHandler> result)
        {
            foreach (var child in adapter.Children)
            {
                ...
                {
                    CollectPhysicalChildren(child, result);
                }
            }
        }
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        private void InsertElement(int siblingIndex, RenderTreeFrame[] frames, int frameIndex, int componentId, RenderBatch batch, HashSet<int> processedComponentIds)
        {
            ...
            for (var descendantIndex = frameIndex + 1; descendantIndex < endIndexExcl; descendantIndex++)
            {
                ...
                {
                    firstChildFrameIndex = descendantIndex;
                    break;
                }
            }
            ...
            if (firstChildFrameIndex < endIndexExcl)
                InsertFrameRange(batch, componentId, childIndex: 0, frames, firstChildFrameIndex, endIndexExcl, processedComponentIds);
        }
